...
class LogIn extends GawibawiboMain implements ActionListener {
	...
	protected static PlayerInfo info = new PlayerInfo();
...
//	public static PlayerInfo dto;
	...
	void loginFrame() {
//		System.out.println(date);						// 출력까진 됨
		info.setLoginTime(date);						// dto로 보낸게 날아가버려서 파일에 저장이 안됨
					...
					outer:while ((str = br.readLine()) != null) {
						if (str.startsWith("password")) {
							...
							if (userpass.equals(dto.getPassword())) {
								flag = true;
								JOptionPane.showMessageDialog(null, "반갑습니다" + dto.getuserId() + "님 즐거운하루되세요.", "WELLCOM!!" + id, JOptionPane.PLAIN_MESSAGE);
